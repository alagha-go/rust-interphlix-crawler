...
pub fn get_episodes(html: &String) -> Vec<(String, String, String)> {
    ...
    let mut episodes: Vec<(String, String, String)> = Vec::new();

    for document in documents {
        let image_url = document.get_by_name(&"img".to_owned()).unwrap().attribute(&"src".to_owned()).unwrap();
        let episode_number = document.get_by_class(&"episode-number".to_owned()).unwrap().get_text().unwrap();
        let name = document.get_by_class(&"film-name".to_owned()).unwrap().child_attribute(&"title".to_owned()).unwrap();
        episodes.push((episode_number, name, image_url));
    }

    episodes.into_iter().rev().collect::<Vec<(String, String, String)>>()
}