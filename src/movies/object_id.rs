...
impl ObjectId {
    ...
    pub fn timestamp(&self) -> SystemTime {
        ...
        let sub_duration = time.duration_since(SystemTime::UNIX_EPOCH).expect("system clock is before 1970");
        let add_duration = std::time::Duration::new(seconds_since_epoch, 0);
        time.sub(sub_duration).add(add_duration)
    }
    ...
    pub fn from_hex(string: &String) -> Result<Self, hex::FromHexError> {
        Ok(Self::from_bytes(hex::decode(string)?.try_into().unwrap()))
    }
